...
<details>
<summary>駭客技術說明 </summary>  


 - **蜜罐 (Honeypot)**：一個假系統或網絡，設計用來引誘駭客，如同誘餌陷阱，研究他們的手法或轉移他們對真實目標的注意力。故事中，它能偵測安全設施中的未經授權探測器。

 - **DNS 毒害（DNS Poisoning）**：破壞系統通訊錄（DNS 快取），將使用者導向假網站，例如竄改電話簿將電話撥錯號碼。這裡用來誤導網路流量。   

- **偽造憑證（Forging Certificates）**：製作假數位身分證（憑證）冒充受信任網站或伺服器，如偽造護照以取得未經授權的存取權限。在小說中，它使得偽裝安全連線成為可能。   

- **流氓AP（Rogue AP）**：攻擊者設置的假 Wi-Fi 熱點，誘使裝置連接，從而攔截資料——就像假的公共 Wi-Fi 誘使使用者進入咖啡廳。    

- **DOS（Denial of Service 阻斷服務）**：以大量流量淹沒系統，導致系統崩潰，就像用無盡電話堵塞電話線，讓任何人無法接通。在這裡，它會暫時中斷服務。   

- **中間人攻擊（Man-in-the-Middle Attack）**：一種駭客技術，攻擊者秘密攔截並可能改變雙方（例如使用者與伺服器）之間的通訊，例如冒充中繼站竊聽對話。在小說中，它被用來監視網路流量而不被察覺。   

- **SSL 剝離技術 (SSL Stripping)**：一種將安全（加密）網路連線降級為未加密網路連線的方法，誘使使用者以明文傳送資料。在這裡，它有助於避免攔截時的安全警報。   

- **特洛伊木馬 (Trojan Horse)**：偽裝成合法的惡意軟體（例如軟體更新），如傳說中藏匿士兵的木馬。故事中，它被植入以持續存取裝置。   

- **Sniffer**：一種能擷取並分析網路資料封包的工具，就像隱藏麥克風在電話線上錄製對話一樣。這裡用來監控中間人（MITM）設定中的流量。   

- **AI 代理（裝置中）(AI Agent in Devices)**：一個利用 AI 獨立執行任務（例如資料收集）的自動化程式，如智慧機器人助理在感染裝置上執行隱藏任務。  
...
他啟動了裝置。它開始廣播一個強大的、偽造的信號，完美地模仿了官方的「FIA_GUEST_LVL2」網絡ID。然後，他觸發了去身份驗證的洪水攻擊——一陣短暫的、目標明確的電子噪音，會導致真實網絡出現短暫的局部中斷，迫使附近的設備自動搜索更強的信號。
