...
**面紗協議（聯邦黑色行動保障）**  
針對陷入危險的特工的機密緊急措施： 
- **閃避功能**：觸發一場假象災難（例如爆炸），模擬死亡，讓特工能離線逃脫追捕。 
- **生存功能**：生物抑制劑能誘發類似昏迷的狀態，掩蓋生命徵象（掃描時平線）並穩定傷口以助恢復。 
- **啟動**：單側透過植入物/信標;設計用來否認，甚至用假的「身體」證據欺騙盟友。 
- **缺點**：長期副作用（例如器官拉傷）;啟動後的支援常被官僚體系拒絕，導致孤立。
