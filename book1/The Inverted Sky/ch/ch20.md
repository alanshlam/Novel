...
<details>
<summary> The Network Worm </summary>
...
**How it works:**
- **Planting**: Delivered via a tiny subdermal tracker disguised as an ordinary humidity probe in the server room’s maintenance port.
- **Dormant Phase**: Once inside, the worm remains completely silent and hidden. It duplicates itself with evolving digital signatures so it blends perfectly into normal environmental telemetry data (temperature logs, humidity readings, etc.). Victor’s air-gapped server only allows external transmission when the owner himself opens files or checks off-site backups.
- **Activation Trigger**: The worm only “wakes up” the instant Victor personally accesses the targeted files (e.g., the Ethan Hale archive and its backup). This is the single moment the server briefly lowers its firewall.
- **Exfiltration**: It then quietly cracks the encryption, bypasses the firewall, and leaks the entire ledger in tiny encrypted micro-bursts through the weather forum’s steganographic images — invisible to normal monitoring.
...
<details>
<summary> Stuxnet Technical Details </summary>
...
- **Air-gapped targeting**: It was built to attack systems not connected to the internet. It spread primarily through infected USB drives.
- **Zero-day exploits**: Used four previously unknown Windows vulnerabilities — an unprecedented number at the time.
- **Siemens PLC attack**: It specifically targeted Siemens Step7 programmable logic controllers (PLCs) used to control industrial centrifuges.
- **Precision sabotage**: Once inside, it subtly altered the speed of the uranium enrichment centrifuges (making them spin too fast or too slow), causing them to tear themselves apart, while simultaneously feeding false “normal operation” data back to the monitoring systems.
- **Stealth & legitimacy**: It used stolen digital certificates from Realtek and JMicron to appear as legitimate Microsoft-signed software.
- **Self-replication**: Highly modular and self-propagating within the target network, but remained dormant until it found its exact target configuration.
- **Self-destruction**: After completing its mission, it was programmed to delete itself to reduce the chance of detection.
...
---
[Previous Chapter](./ch19.md)|[Next Chapter](./ch21.md)
